...
var LayaUISample=(function(){
	function LayaUISample(){
		...
		//"fixedwidth";
		this._screenMode="vertical";
		//"none";
		this._alignV="middle";
		//"middle";
		this._alignH="center";
		MiniAdpter.init(true,true);
		Laya.init(this._width,this._height);
		Laya.stage.scaleMode=this._scaleMode;
		Laya.stage.screenMode=this._screenMode;
		Laya.stage.alignV=this._alignV;
		Laya.stage.alignH=this._alignH;
...
var MsgMgr=(function(){
		...
		if(MiniAdpter.isZiYu && MiniAdpter.isPosMsgYu){
			this.wx.onMessage(function(message){
				...
				if(message['isLoad']=="filedata"){
					...
					_$this.getFriendCloudStorage(message['kvArr']);
	...
	__proto.beginLoad=function(){
		Laya.loader.load("res/atlas/ui/wxrank.atlas",Handler.create(this,this.onLoaded));
	}
	...
	__proto.updateRank=function(rankObj){
		if(rankObj.length<=1){
			var obj=rankObj[0];
			var arr=[obj];
	...
	__proto.getFriendCloudStorage=function(_kvArr){
		var _$this=this;
		this.wx.getFriendCloudStorage({
			keyList:_kvArr,
			success:function (res){
				_$this.updateRank(res.data);
			},
			fail:function (res){}
		});
	}
	...
	__proto.getUserCloudStorage=function(_kvArr){
	...
	__getset(1,MsgMgr,'instance',function(){
		return this._instance=this._instance|| new MsgMgr();
	});
...
var MiniAdpter=(function(){
	...
	MiniAdpter.init=function(isPosMsg,isSon){
		...
		try{
			laya.webgl.resource.WebGLCanvas.premulAlpha=true;
			}catch(e){
		}
...
//class ui.FriendRankUI extends laya.ui.View
var FriendRankUI=(function(_super){
	function FriendRankUI(){
		this.panelbox=null;
		this.friendlist=null;
		FriendRankUI.__super.call(this);
	}

	__class(FriendRankUI,'ui.FriendRankUI',_super);
	var __proto=FriendRankUI.prototype;
	__proto.createChildren=function(){
...
*@author
*/
//class view.FriendRank extends ui.FriendRankUI
var FriendRank=(function(_super){
	function FriendRank(){
		this._instance=null;
		FriendRank.__super.call(this);
		this.friendlist.vScrollBarSkin="";
		this.friendlist.renderHandler=new Handler(this,this.updatelist);
		this.friendlist.array=[];
	}

	__class(FriendRank,'view.FriendRank',_super);
	var __proto=FriendRank.prototype;
	__proto.initNum=function(arr){
		arr=this.setScore(arr);
	...
	__proto.updatelist=function(cell,index){
		...
		var name=cfg.nickname;
		var score=cfg.score;
		var headUrl=cfg.avatarUrl;
		var rankNum=cfg.rank;
		var ele_floor_img=cell.getChildByName("floorImg");
		var ele_head_img=cell.getChildByName("headBox").getChildByName("headimg");
		var ele_rank_img=cell.getChildByName("rankimg");
		var ele_title_txt=cell.getChildByName("titletxt");
		var ele_name_txt=cell.getChildByName("nametxt");
		var ele_rank_txt=cell.getChildByName("ranktxt");
		var ele_score_txt=cell.getChildByName("scoretxt");
		ele_head_img.skin=headUrl;
		ele_name_txt.text=name;
		ele_title_txt.text=this.getPlayerTitle(score,1000);
	...
	*/
	__proto.setScore=function(_arr){
		for(var i=0;i<_arr.length;i++){
			var score=parseInt(_arr[i].KVDataList[0].value);
			_arr[i].score=score;
		}
		this.sortArr(_arr);
		this.setRankNum(_arr);
		return _arr;
	}

	__proto.sortArr=function(_arr){
		_arr.sort(function(a,b){
			return (a.score>b.score)?-1:1
		});
	}

	__proto.setRankNum=function(_arr){
		for(var i=0;i<_arr.length;i++){
			_arr[i].rank=i+1;
		}
	}

	__proto.getPlayerTitle=function(weight,maxWeight){
		var lvNum=maxWeight/10;
		var str;
		if(weight>=maxWeight){
			str="盖世无双";
		}
	...
	__getset(1,FriendRank,'instance',function(){
		return this._instance=this._instance||new FriendRank();
	},ui.FriendRankUI._$SET_instance);
