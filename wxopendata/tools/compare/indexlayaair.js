...
var document = document || window.document||(window.document = {});
/***********************************/
...
(function(window,document,Laya){
	...
	*@private
	*/
	//class global.laya.utils.RunDriver
	...
	var ___Laya=(function(){
		...
		*/
		...
		Laya.init=function(width,height,__plugins){
			...
			Laya._isinit=true;
			var app=getApp();
app.contextFrom=global.laya.resource.Context.prototype;
Browser.__init__();
			Context.__init__();
			Graphics.__init__();
	...
	*/
	...
	var TestSimple=(function(){
		function TestSimple(){
			...
			DebugTxt.init();
			this.picPath="res/boy.png";
			this.aniPath="res/hero1Ani.ani";
			this.soundPath="sound/shot.mp3";
			this.musicPath="sound/music.mp3";
			URL.rootPath="";
			URL.basePath="";
			URL.basePath="https://onewaymyway.github.io/tempres/xiaochengxu/";
			console.log("stage:",Laya.stage.width,Laya.stage.height);
			this.showStageInfo();
			this.testLoadAni();
			this.testSocket();
			this.testLoadSmallPics();
			this.testInput();
		}

		__class(TestSimple,'test.TestSimple');
		var __proto=TestSimple.prototype;
		__proto.testInput=function(){
			var sp;
			sp=new Sprite;
			sp.graphics.drawRect(0,0,100,20,"#ff0000");
			sp.pos(200,200);
			...
			input=new Input;
			input.pos(200,200);
			input.text="laya input";
			input.color="#ffff00";
			Laya.stage.addChild(input);
		}

		__proto.testLoadSmallPics=function(){
			var atl;
			atl=new AtlasLoadTools();
			atl.loadAtlas("res/atlas/comp.json",Handler.create(this,this.onAtlasLoaded));
		}

		__proto.onAtlasLoaded=function(){
			console.log("onAtlasLoaded");
			var page;
			page=new TestPageSimpleUI();
			page.scale(0.5,0.5);
			Laya.stage.addChild(page);
			return;
			var tex;
			tex=Loader.getRes("comp/image.png");
			var sp;
			sp=new Sprite();
		...
		__proto.testSocket=function(){
			var socket;
			socket=new Socket();
			socket.connectByUrl("ws://orzooo.com:9909");
			socket.on("open",this,this.onConnect);
			socket.on("message",this,this.onMessage);
			socket.on("error",this,this.onErr);
			socket.on("close",this,this.onClose);
		}

		__proto.onConnect=function(){
			DebugTxt.dTrace('socket connect');
		}

		__proto.onMessage=function(data){
			DebugTxt.dTrace('socket onMessage',data);
		}

		__proto.onErr=function(e){
			DebugTxt.dTrace('socket onErr',e);
		}

		__proto.onClose=function(){
			DebugTxt.dTrace('socket onClose');
		}

		//Laya.timer.once(1000,this,closeLater);
		__proto.testLoadAni=function(){
			Laya.loader.load(this.aniPath,new Handler(this,this.aniLoaded),null,"json");
		}

		__proto.aniLoaded=function(){
			SoundManager.playMusic(this.musicPath,2);
			console.log("ani:",Loader.getRes(this.aniPath));
		}

		__proto.showStageInfo=function(){
			var sp;
			sp=new Sprite();
			sp.graphics.fillText("Stage:"+Laya.stage.width+","+Laya.stage.height,0,0,null,"#ff0000","left");
			Laya.stage.addChild(sp);
			sp.zOrder=999;
		}

		__proto.testMouseEvents=function(){
			this.mouseSp=new Sprite();
			this.mouseSp.graphics.drawRect(0,0,20,20,"#ff0000");
			this.mouseSp.scale(0.5,2);
			this.mouseSp.pivot(10,10);
			this.mouseSp.graphics.fillText("orz",0,0,"","#00ffff","left");
			Laya.stage.addChild(this.mouseSp);
			Laya.timer.loop(100,this,this.rotateSp,[this.mouseSp]);
			Laya.stage.on("mousedown",this,this.onEvent);
			Laya.stage.on("mouseup",this,this.onEvent);
		}

		//Laya.stage.on(Event.MOUSE_MOVE,this,onEvent);
		__proto.rotateSp=function(sp){
			sp.rotation++;
		}

		__proto.onEvent=function(e){
			console.log("event:",e.type,e.stageX,e.stageY);
			if (e.type=="mouseup"){
				if (this.mouseSp){
					this.mouseSp.pos(e.stageX,e.stageY);
					this.mouseSp.rotation=0;
				}
				SoundManager.playSound(this.soundPath,3);
			}
		}

		__proto.testTexture=function(){
			var tex;
			tex=Loader.getRes(this.picPath);
			var sp;
			sp=new Sprite();
			sp.pos(0,0);
			...
			sp.graphics.drawTexture(tex,0,0);
			Laya.stage.addChild(sp);
			Laya.timer.loop(100,this,this.randomPos,[sp]);
		}

		__proto.testGraphic=function(){
			var sp;
			sp=new Sprite();
		...
		//Laya.timer.loop(100,this,randomPos,[sp]);
		__proto.randomPos=function(sp){
			sp.x=200 *Math.random();
			sp.y=200 *Math.random();
		}
	...
	*/
	//class global.laya.events.EventDispatcher
	var EventDispatcher=(function(){
		...
		*/
		...
		EventDispatcher.__init$=function(){
			...
			/**@private */
			//class EventHandler extends global.laya.utils.Handler
	...
	*/
	//class global.laya.utils.Handler
	...
	*/
	//class global.laya.display.BitmapFont
	...
	*/
	//class global.laya.display.css.Style
	var Style=(function(){
		function Style(){
			this.alpha=1;
			this.visible=true;
	...
	*/
	//class global.laya.display.css.Font
	var Font=(function(){
		function Font(src){
			this._type=0;
			this._weight=0;
	...
	*@private
	*/
	//class global.laya.display.css.TransformInfo
	...
	*@see global.laya.display.Sprite#graphics
	*/
	//class global.laya.display.Graphics
	var Graphics=(function(){
		...
		*/
		...
		Graphics.__init__=function(){
			if (Render.isConchNode){
				var from=global.laya.display.Graphics.prototype;
	...
	*Graphic bounds数据类
	*/
	//class global.laya.display.GraphicsBounds
	...
	*/
	//class global.laya.events.Event
	var Event=(function(){
		function Event(){
			//this.type=null;
	...
	*/
	//class global.laya.events.KeyBoardManager
	var KeyBoardManager=(function(){
		...
		KeyBoardManager._addEvent=function(type){
			Browser.document.addEventListener(type,function(e){
				global.laya.events.KeyBoardManager._dispatch(e,type);
			},true);
		}
	...
	*/
	//class global.laya.events.MouseManager
		...
		*/
		...
		__proto.onMouseDown=function(ele){
			if (Input.isInputting && Laya.stage.focus && Laya.stage.focus["focus"] && !Laya.stage.focus.contains(this._target)){
				...
				if ((new_input instanceof global.laya.display.Input )&& new_input.multiline==pre_input.multiline)
					pre_input['_focusOut']();
				else
		...
		__proto.hitTest=function(sp,mouseX,mouseY){
			...
			if ((sp.hitArea instanceof global.laya.utils.HitArea )){
	...
	*/
	//class global.laya.events.TouchManager
	...
	*/
	//class global.laya.filters.Filter
	var Filter=(function(){
		function Filter(){
			this._action=null;
		}
	...
	*/
	//class global.laya.filters.ColorFilterAction
	...
	*计算贝塞尔曲线的工具类。
	*/
	//class global.laya.maths.Bezier
	var Bezier=(function(){
		function Bezier(){
	...
	*凸包算法。
	*/
	//class global.laya.maths.GrahamScan
	...
	*/
	//class global.laya.maths.MathUtil
	...
	*/
	//class global.laya.maths.Matrix
	var Matrix=(function(){
	...
	*/
	//class global.laya.maths.Rectangle
	...
	*/
	//class global.laya.media.SoundManager
	...
	*/
	//class global.laya.net.LocalStorage
	...
	*/
	//class global.laya.renders.Render
	var Render=(function(){
		function Render(width,height){
			...
			var isWebGl=global.laya.renders.Render.isWebGL;
	...
	*渲染环境
	*/
	//class global.laya.renders.RenderContext
	...
	*精灵渲染器
	*/
	//class global.laya.renders.RenderSprite
	...
	*Context扩展类
	*/
	//class global.laya.resource.Context
	var Context=(function(){
		...
		Context.__init__=function(to){
			var from=global.laya.resource.Context.prototype;
	...
	*/
	//class global.laya.resource.ResourceManager
	...
	*/
	//class global.laya.ui.LayoutStyle
	...
	*/
	//class global.laya.utils.Browser
...
Browser.__init__();
		...
		Browser.__init__=function(){
			...
			Browser._window.addEventListener('message',function(e){
				global.laya.utils.Browser._onMessage(e);
			},false);
...
Browser.__init__();
	...
	*对象缓存统一管理类
	*/
	//class global.laya.utils.CacheManger
	...
	*/
	//class global.laya.utils.ClassUtils
	...
	*/
	//class global.laya.utils.Dragging
	...
	*/
	//class global.laya.utils.HitArea
	...
	*/
	//class global.laya.utils.HTMLChar
	...
	*/
	...
	var Utils=(function(){
		...
		Utils.getTransformRelativeToWindow=function(coordinateSpace,x,y){
			var stage=Laya.stage;
			var globalTransform=global.laya.utils.Utils.getGlobalPosAndScale(coordinateSpace);
	...
	*@private
	*/
	//class global.laya.utils.VectorGraphManager
	...
	*/
	//class global.laya.utils.WeakObject
	...
	*@private
	*/
	//class global.laya.utils.WordText
	...
	*@author ww
	*/
	//class global.laya.display.inputwrap.InputWrap
	var InputWrap=(function(){
		...
		__class(InputWrap,'laya.display.inputwrap.InputWrap');
		var __proto=InputWrap.prototype;
		__proto.focus=function(){
			this.dataO.display="normal";
		}

		__proto.blur=function(){
			this.dataO.display="none";
		}

		__proto.select=function(){}
		__proto.init=function(inputData){
			this.dataO=inputData;
			this.dataO.inputFunction=Utils.bind(this._onInput,this);
		}

		__proto._onInput=function(res){
			console.log("_onInput:",res);
			this.value=res.detail.value;
		}

		__proto.freshUI=function(){
			this.dataO.value=this.value;
			this.dataO.top=this._posY+this.dataO.height;
			Mini.freshData();
		}

		__proto.setVisible=function(value){
			this.dataO.display=value?"normal":"none";
		}

		__proto.setForbidEdit=function(value){}
		__proto.setRestrict=function(value){}
		__proto.setScale=function(x,y){}
		__proto.setSize=function(x,y){
			this.dataO.width=x;
			this.dataO.height=y;
		}
		...
		__proto.setColor=function(value){
			this.dataO.color=value;
		}

		__proto.setFontSize=function(value){
			this.dataO.fontSize=value;
		}

		__proto.setFontFace=function(value){}
		__proto.setSelection=function(start,end){}
		InputWrap.createByData=function(dataO){
			var rst;
			rst=new InputWrap();
			rst.init(dataO);
			return rst;
	...
	*@author ww
	*/
	//class global.laya.mini.AtlasLoadTools
	var AtlasLoadTools=(function(){
		function AtlasLoadTools(){
			this.complete=null;
		}

		__class(AtlasLoadTools,'laya.mini.AtlasLoadTools');
		var __proto=AtlasLoadTools.prototype;
		__proto.loadAtlas=function(atlasPath,complete){
			this.complete=complete;
			Laya.loader.load(atlasPath,new Handler(this,this.configFileLoaded,[atlasPath]),null,"json");
		}

		__proto.configFileLoaded=function(atlasPath){
			var _data;
			_data=Loader.getRes(atlasPath);
			var _url;
			_url=atlasPath;
			var frames=_data.frames;
			var cleanUrl=_url.split("?")[0];
			var directory=(_data.meta && _data.meta.prefix)? _data.meta.prefix :cleanUrl.substring(0,cleanUrl.lastIndexOf("."))+"/";
			var pics=_data.pics;
			var atlasURL=URL.formatURL(_url);
			var smallPics=[];
			for (var name in frames){
				var url=URL.formatURL(directory+name);
				smallPics.push(url);
			}
			Laya.loader.load(smallPics,new Handler(this,this.onPicLoaded,[atlasPath]));
		}

		__proto.onPicLoaded=function(){
			if (this.complete){
				this.complete.run();
			}
		}

		return AtlasLoadTools;
	})()
	...
	*@author ww
	*/
	//class global.laya.mini.debug.DebugTxt
	var DebugTxt=(function(){
		function DebugTxt(){}
		__class(DebugTxt,'laya.mini.debug.DebugTxt');
	...
	*@author ww
	*/
	//class global.laya.mini.media.MiniSound
	var MiniSound=(function(){
		function MiniSound(soundID){
			this._sound=null;
			this.src=null;
			this.isPlaying=false;
			this.loops=-1;
			this.completeHandler=null;
			this._sound=wx.createAudioContext(soundID);
			MiniSound._soundDic[soundID]=this;
		}

		__class(MiniSound,'laya.mini.media.MiniSound');
		var __proto=MiniSound.prototype;
		__proto.setSrc=function(src){
			this.src=src;
			this.pause();
		...
		__proto.seek=function(position){
		...
		MiniSound.onAudioEnd=function(dataO){
			console.log("onAudioEnd",dataO);
			if (dataO && dataO.target){
				var tID;
				tID=dataO.target.id;
				var tSound;
				tSound=MiniSound._soundDic[tID];
				if (tSound){
					tSound._stop();
				}
			}
	...
	*@author ww
	*/
	//class global.laya.mini.Mini
	var Mini=(function(){
		function Mini(){}
		__class(Mini,'laya.mini.Mini');
		__getset(1,Mini,'isDebug',function(){
			return false;
			return Mini.app && Mini.app.isDebugMode;
		});

		Mini.init=function(){
			var _windos=window;
			if (_windos.HTMLCanvasElement)return;
			MiniXMLHttpRequest;
			Loader.prototype._loadImage=MiniImageLoader.prototype._loadImage;
			Loader.prototype._loadImage=MiniImageLoader.prototype._loadImage2;
			MouseManager.prototype.initEvent=MiniMouseManager.prototype.initEvent;
			var tApp;
			tApp=getApp();;
			Mini.app=tApp;
			Mini.app.mouseHandler=MiniMouseManager.addEvent;
			Mini.app.MiniXMLHttpRequest=MiniXMLHttpRequest;
			MiniMeasureText.I=new MiniMeasureText();
			Mini.app.measureText=MiniMeasureText.contextMeasureText;
			MiniSoundManager.init();
			Mini.app.MiniSocket=new MiniWebsocket();
			Mini.that=Mini.app.that;
			Mini.pageData=Mini.app.pageData;
		}

		Mini.freshData=function(){
			Mini.that.setData(Mini.pageData);
		}

		Mini.setData=function(dataO){
			Mini.that.setData(dataO);
		}
	...
	*@author ww
	*/
	//class global.laya.mini.MiniImageLoader
	var MiniImageLoader=(function(){
		function MiniImageLoader(){}
		__class(MiniImageLoader,'laya.mini.MiniImageLoader');
		var __proto=MiniImageLoader.prototype;
		/**
		*@private
		*加载图片资源。
		*@param url 资源地址。
		*/
		__proto._loadImage=function(url){
			...
			var _this=this;
			var image;
			wx.getImageInfo({src:url,success:function (res){
					var bmp;
					bmp=new MiniBitmap();
					bmp.initByData(url,res);
					_this.onLoaded(bmp);
			}});
		}

		__proto._loadImage2=function(url){
			url=URL.formatURL(url);
			var _this=this;
			wx.downloadFile({url:url,success:function (res){
					console.log("downloadFile success",res);
					wx.getImageInfo({src:res.tempFilePath,success:function (res){
							var bmp;
							bmp=new MiniBitmap();
							bmp.initByData(url,res);
							_this.onLoaded(bmp);
	...
	*@author ww
	*/
	//class global.laya.mini.MiniMeasureText
	var MiniMeasureText=(function(){
		function MiniMeasureText(){
			this._font=null;
			this._fontSize=12;
		}

		__class(MiniMeasureText,'laya.mini.MiniMeasureText');
		var __proto=MiniMeasureText.prototype;
		__proto.measureText=function(text){
			...
			noChineseCount=0;
			for (i=0;i < len;i++){
				if (!MiniMeasureText.isChinese(text.charAt(i))){
					noChineseCount++;
				}
			}
		...
		__getset(0,__proto,'font',function(){
			...
			this._font=v;
			this._fontSize=MiniMeasureText.getFontSize(v);
		});

		MiniMeasureText.contextMeasureText=function(text,font){
			(font===void 0)&& (font="12px");
			MiniMeasureText.I.font=font;
			return MiniMeasureText.I.measureText(text);
		}

		MiniMeasureText.isChinese=function(str){
			return MiniMeasureText.reg.test(str);
		}

		MiniMeasureText.getFontSize=function(fontStr){
		...
		MiniMeasureText.I=null
		__static(MiniMeasureText,
		['reg',function(){return this.reg=new RegExp("[\\u4E00-\\u9FFF]+","g");}
		]);
		return MiniMeasureText;
	})()
	...
	*@author ww
	*/
	//class global.laya.mini.MiniMouseManager
	var MiniMouseManager=(function(){
		function MiniMouseManager(){
			this._stage=null;
			this._point=null;
			this._tTouchID=NaN;
		}

		__class(MiniMouseManager,'laya.mini.MiniMouseManager');
		var __proto=MiniMouseManager.prototype;
		__proto.initEvent=function(e,nativeEvent){
			var _this=this;
			_this._event._stoped=false;
			_this._event.nativeEvent=nativeEvent || e;
		...
		MiniMouseManager.addEvent=function(e){
			var _this=MouseManager.instance;
			switch (e.type){
				case "touchstart":
					_this.mouseDownTime=Browser.now();
					break ;
			...
			MouseManager.instance["_eventList"].push(e);
		}

		return MiniMouseManager;
	})()
	...
	*详细教程及声音格式请参考：http://ldc.layabox.com/doc/?nav=ch-as-1-7-0
	*/
	//class global.laya.mini.MiniSoundManager
	var MiniSoundManager=(function(){
		function MiniSoundManager(){};
		__class(MiniSoundManager,'laya.mini.MiniSoundManager');
		/**
		*背景音乐和所有音效是否静音。
		*/
		__getset(1,MiniSoundManager,'muted',function(){
			return MiniSoundManager._muted;
			},function(value){
			if (value){
				MiniSoundManager.stopAllSound();
			}
			MiniSoundManager.musicMuted=value;
			MiniSoundManager._muted=value;
		});

		/**
		*背景音乐（不包括音效）是否静音。
		*/
		__getset(1,MiniSoundManager,'musicMuted',function(){
			return MiniSoundManager._musicMuted;
			},function(value){
			if (value){
				if (MiniSoundManager._tMusic)
					MiniSoundManager.stopSound(MiniSoundManager._tMusic);
				MiniSoundManager._musicMuted=value;
				}else {
				MiniSoundManager._musicMuted=value;
				if (MiniSoundManager._tMusic){
					MiniSoundManager.playMusic(MiniSoundManager._tMusic);
				}
			}
		});
		...
		*所有音效（不包括背景音乐）是否静音。
		*/
		__getset(1,MiniSoundManager,'soundMuted',function(){
			return MiniSoundManager._soundMuted;
			},function(value){
			MiniSoundManager._soundMuted=value;
		});

		MiniSoundManager.init=function(){
			MiniSoundManager._musicSound=new MiniSound("musicAudio");
			MiniSoundManager._effectSound=new MiniSound("soundAudio");
			Mini.app.SoundEndHandler=MiniSound.onAudioEnd;
			var replaceFuns;
			replaceFuns=[
			"playSound",
			"playMusic",
			...
			"_setVolume"];
			var i=0,len=0;
			len=replaceFuns.length;
			var tFun;
			for (i=0;i < len;i++){
				tFun=replaceFuns[i];
				SoundManager[tFun]=MiniSoundManager[tFun];
			}
		}

		MiniSoundManager.playSound=function(url,loops,complete,soundClass,startTime){
			(loops===void 0)&& (loops=1);
			(startTime===void 0)&& (startTime=0);
			url=URL.formatURL(url);
			MiniSoundManager._effectSound.setSrc(url);
			MiniSoundManager._effectSound.loops=loops;
			MiniSoundManager._effectSound.completeHandler=complete;
			MiniSoundManager._effectSound.seek(startTime);
			MiniSoundManager._effectSound.play();
		}

		MiniSoundManager.destroySound=function(url){}
		MiniSoundManager.playMusic=function(url,loops,complete,startTime){
			(loops===void 0)&& (loops=0);
			(startTime===void 0)&& (startTime=0);
			url=URL.formatURL(url);
			MiniSoundManager._musicSound.setSrc(url);
			MiniSoundManager._musicSound.loops=loops;
			MiniSoundManager._musicSound.completeHandler=complete;
			MiniSoundManager._musicSound.seek(startTime);
			MiniSoundManager._musicSound.play();
		}

		MiniSoundManager.stopSound=function(url){
			url=URL.formatURL(url);
			if (MiniSoundManager._musicSound.src==url)MiniSoundManager._musicSound.stop();
			if (MiniSoundManager._effectSound.src==url)MiniSoundManager._effectSound.stop();
		}

		MiniSoundManager.stopAll=function(){
			MiniSoundManager._tMusic=null;
			MiniSoundManager._musicSound.stop();
			MiniSoundManager._effectSound.stop();
		}

		MiniSoundManager.stopAllSound=function(){
			MiniSoundManager._effectSound.stop();
		}

		MiniSoundManager.stopMusic=function(){
			MiniSoundManager._tMusic=null;
			MiniSoundManager._musicSound.stop();
		}

		MiniSoundManager.setSoundVolume=function(volume,url){}
		MiniSoundManager.setMusicVolume=function(volume){}
		MiniSoundManager._setVolume=function(url,volume){}
		MiniSoundManager.musicVolume=1;
		MiniSoundManager.soundVolume=1;
		MiniSoundManager.playbackRate=1;
		MiniSoundManager.useAudioMusic=true;
		MiniSoundManager._muted=false;
		MiniSoundManager._soundMuted=false;
		MiniSoundManager._musicMuted=false;
		MiniSoundManager._tMusic=null;
		MiniSoundManager._musicChannel=null;
		MiniSoundManager._channels=[];
		MiniSoundManager._autoStopMusic=false;
		MiniSoundManager._blurPaused=false;
		MiniSoundManager._isActive=true;
		MiniSoundManager._musicLoops=0;
		MiniSoundManager._musicPosition=0;
		MiniSoundManager._musicCompleteHandler=null;
		MiniSoundManager._soundClass=null
		MiniSoundManager.autoReleaseSound=true;
		MiniSoundManager._musicSound=null
		MiniSoundManager._effectSound=null
		return MiniSoundManager;
	})()
	...
	*@author ww
	*/
	//class global.laya.mini.MiniWebsocket
	var MiniWebsocket=(function(){
		function MiniWebsocket(){
			...
			this.onerror=null;
			var _$this=this;
			wx.onSocketOpen(onSocketOpen=function(res){
				if (_$this.onopen!=null)
					_$this.onopen(res);
			});
			wx.onSocketError(onSocketError=function(res){
				if (_$this.onerror!=null)
					_$this.onerror(res);
			});
			wx.onSocketMessage(onSocketMessage=function(res){
				if (_$this.onmessage!=null)
					_$this.onmessage(res);
			});
			wx.onSocketClose(onSocketClose=function(res){
				if (_$this.onclose!=null)
					_$this.onclose(res);
			});
		}

		__class(MiniWebsocket,'laya.mini.MiniWebsocket');
		var __proto=MiniWebsocket.prototype;
		__proto.connect=function(url,protocols){
			var paramO;
			paramO={};
			paramO.url=url;
			wx.connectSocket(paramO);
			return this;
		}
		...
		__proto.close=function(){
			var paramO;
			paramO={};
			paramO.reason="user close";
			wx.closeSocket(paramO);
		}
	...
	*@author ww
	*/
	//class global.laya.mini.MiniXMLHttpRequest
	var MiniXMLHttpRequest=(function(){
		...
		__class(MiniXMLHttpRequest,'laya.mini.MiniXMLHttpRequest');
		...
		__proto.send=function(data){
			...
			requestO={
				...
				success:function (res){
					_$this.status=res.statusCode;
					_this.data=res.data;
					_this.responseText=res.data;
					if (_$this.onload!=null){
						_$this.onload(this);
					}
			}};
			if (data){
	...
	*/
	//class global.laya.display.Node extends global.laya.events.EventDispatcher
	...
	*/
	//class global.laya.display.css.CSSStyle extends global.laya.display.css.Style
	...
	*使用Audio标签播放声音
	*/
	//class global.laya.media.h5audio.AudioSound extends global.laya.events.EventDispatcher
	...
	*/
	//class global.laya.media.SoundChannel extends global.laya.events.EventDispatcher
	...
	*/
	//class global.laya.media.Sound extends global.laya.events.EventDispatcher
	...
	*web audio api方式播放声音
	*/
	//class global.laya.media.webaudio.WebAudioSound extends global.laya.events.EventDispatcher
	var WebAudioSound=(function(_super){
		...
		*/
		__proto.load=function(url){
			...
			var request=new (getApp().MiniXMLHttpRequest)();
	...
	*/
	//class global.laya.net.HttpRequest extends global.laya.events.EventDispatcher
	var HttpRequest=(function(_super){
		function HttpRequest(){
			this._responseType=null;
			this._data=null;
			HttpRequest.__super.call(this);
			this._http=new (getApp().MiniXMLHttpRequest)();
		}
	...
	*/
	//class global.laya.net.Loader extends global.laya.events.EventDispatcher
		...
		*/
		__proto.load=function(url,type,cache,group,ignoreCache){
			...
			if (Loader.parserMap[type] !=null){
				...
				if (((Loader.parserMap[type])instanceof global.laya.utils.Handler ))Loader.parserMap[type].runWith(this);
		...
		*/
		...
		Loader.clearRes=function(url,forceDispose){
			...
			if (arr){
				...
				if (res){
					delete Loader.loadedMap[url];
					if ((res instanceof global.laya.resource.Texture )&& res.bitmap)(res).destroy(forceDispose);
				}
			}
		}
	...
	*@see global.laya.net.Loader
	*/
	//class global.laya.net.LoaderManager extends global.laya.events.EventDispatcher
	var LoaderManager=(function(_super){
		...
		*@param arr 简单：["a.png","b.png"]，复杂[{url:"a.png",type:Loader.IMAGE,size:100,priority:1},{url:"b.json",type:Loader.JSON,size:50,priority:1}]*/
		...
		LoaderManager.__init$=function(){
			//class ResInfo extends global.laya.events.EventDispatcher
	...
	*/
	//class global.laya.filters.ColorFilter extends global.laya.filters.Filter
	...
	*/
	//class global.laya.net.Socket extends global.laya.events.EventDispatcher
		...
		*/
		__proto.connectByUrl=function(url){
			...
			if (!this.protocols || this.protocols.length==0){
				this._socket=(getApp().MiniSocket.connect)(url);
				}else {
				this._socket=(getApp().MiniSocket.connect)(url,this.protocols);
			}
	...
	*/
	//class global.laya.resource.Resource extends global.laya.events.EventDispatcher
	...
	*/
	//class global.laya.resource.Texture extends global.laya.events.EventDispatcher
	var Texture=(function(_super){
		...
		__proto.setTo=function(bitmap,uv){
			...
			if (bitmap){
				...
				if (this._loaded){
					...
					var bm=bitmap;
					if ((bm instanceof global.laya.resource.HTMLImage )&& bm.image)
		...
		*/
		...
		Texture.create=function(source,x,y,width,height,offsetX,offsetY,sourceWidth,sourceHeight){
			...
			var btex=(source instanceof global.laya.resource.Texture );
	...
	*/
	//class global.laya.ui.AutoBitmap extends global.laya.display.Graphics
	var AutoBitmap=(function(_super){
		...
		*/
		...
		/**
		*对象的纹理资源。
		*@see global.laya.resource.Texture
		*/
		__getset(0,__proto,'source',function(){
			return this._source;
			},function(value){
	...
	*/
	//class global.laya.ui.UIEvent extends global.laya.events.Event
	...
	*package
	*{
		*import global.laya.display.Sprite;
		*import global.laya.events.Event;
		*
	...
	*function onInit()
	*{
		*sprite=new global.laya.display.Sprite();//创建一个 Sprite 类的实例对象 sprite 。
		*sprite.loadImage("resource/ui/bg.png");//加载并显示图片。
		*sprite.x=200;//设置 sprite 对象相对于父容器的水平方向坐标值。
		*sprite.y=200;//设置 sprite 对象相对于父容器的垂直方向坐标值。
		*sprite.pivotX=0;//设置 sprite 对象的水平方法轴心点坐标。
		...
		*Laya.stage.addChild(sprite);//将此 sprite 对象添加到显示列表。
		*sprite.on(Event.CLICK,this,onClickSprite);//给 sprite 对象添加点击事件侦听。
		*shape=new global.laya.display.Sprite();//创建一个 Sprite 类的实例对象 sprite 。
		*shape.graphics.drawRect(0,0,100,100,"#ccff00","#ff0000",2);//绘制一个有边框的填充矩形。
		*shape.x=400;//设置 shape 对象相对于父容器的水平方向坐标值。
		*shape.y=200;//设置 shape 对象相对于父容器的垂直方向坐标值。
		*shape.width=100;//设置 shape 对象的宽度。
		...
		*shape.pivotY=50;//设置 shape 对象的垂直方法轴心点坐标。
		*Laya.stage.addChild(shape);//将此 shape 对象添加到显示列表。
		*shape.on(global.laya.events.Event.CLICK,this,onClickShape);//给 shape 对象添加点击事件侦听。
		*}
	...
	*
	*@example
	*import Sprite=global.laya.display.Sprite;
	*class Sprite_Example {
		...
		*private onInit():void {
			...
			*this.sprite.on(global.laya.events.Event.CLICK,this,this.onClickSprite);//给 sprite 对象添加点击事件侦听。
			...
			*this.shape.on(global.laya.events.Event.CLICK,this,this.onClickShape);//给 shape 对象添加点击事件侦听。
			*}
		...
		*}
	*/
	//class global.laya.display.Sprite extends global.laya.display.Node
		...
		*/
		__proto._getBoundPointsM=function(ifRotate){
			...
			for (var i=0,n=__childs.length;i < n;i++){
				child=__childs [i];
				if ((child instanceof global.laya.display.Sprite )&& child.visible==true){
		...
		*/
		...
		__getset(0,__proto,'filters',function(){
			...
			if (value && value.length > 0){
				if (!this._getBit(0x1))this._setUpNoticeType(0x1);
				if (!(Render.isWebGL && value.length==1 && (((value[0])instanceof global.laya.filters.ColorFilter )))){
	...
	*audio标签播放声音的音轨控制
	*/
	//class global.laya.media.h5audio.AudioSoundChannel extends global.laya.media.SoundChannel
	...
	*/
	//class global.laya.media.webaudio.WebAudioSoundChannel extends global.laya.media.SoundChannel
	...
	*/
	//class global.laya.resource.Bitmap extends global.laya.resource.Resource
	...
	*/
	//class global.laya.ui.Component extends global.laya.display.Sprite
	...
	*/
	//class global.laya.display.AnimationPlayerBase extends global.laya.display.Sprite
	...
	*package
	*{
		*import global.laya.display.Text;
	...
	*function onInit()
	*{
		*var text=new global.laya.display.Text();//创建一个 Text 类的实例对象 text 。
		*text.text="这个是一个 Text 文本示例。";
		*text.color="#008fff";//设置 text 的文本颜色。
		*text.font="Arial";//设置 text 的文本字体。
		*text.bold=true;//设置 text 的文本显示为粗体。
	...
	*class Text_Example {
		*constructor(){
			...
			*}
		*private onInit():void {
			*var text:global.laya.display.Text=new global.laya.display.Text();//创建一个 Text 类的实例对象 text 。
			*text.text="这个是一个 Text 文本示例。";
			*text.color="#008fff";//设置 text 的文本颜色。
			*text.font="Arial";//设置 text 的文本字体。
			*text.bold=true;//设置 text 的文本显示为粗体。
		...
		*}
	*/
	//class global.laya.display.Text extends global.laya.display.Sprite
	var Text=(function(_super){
		...
		*/
		__proto.renderText=function(begin,visibleLineCount){
			...
			graphics.clear(true);
			var ctxFont=(this.italic ? "italic " :"")+(this.bold ? "bold " :"")+this.fontSize+"px "+(Browser.onIPhone ? (global.laya.display.Text._fontFamilyMap[this.font] || this.font):this.font);
		...
		*@see global.laya.display.css.Font#defaultFamily
		*/
		__getset(0,__proto,'font',function(){
		...
		*/
		...
		Text.setTextRightToLeft=function(){
			...
			global.laya.display.Text.RightToLeft=true;
	...
	*/
	//class global.laya.display.Stage extends global.laya.display.Sprite
...
context.ctx.draw();
	...
	*/
	//class global.laya.resource.FileBitmap extends global.laya.resource.Bitmap
	...
	*/
	//class global.laya.resource.HTMLCanvas extends global.laya.resource.Bitmap
	...
	*@author ww
	*/
	//class global.laya.mini.MiniBitmap extends global.laya.resource.Bitmap
	var MiniBitmap=(function(_super){
		function MiniBitmap(){
			MiniBitmap.__super.call(this);
		}

		__class(MiniBitmap,'laya.mini.MiniBitmap',_super);
		var __proto=MiniBitmap.prototype;
		__proto.initByData=function(src,info){
			this.src=info.path;
			this._w=info.width;
			this._h=info.height;
			if (Mini.isDebug){
	...
	*/
	//class global.laya.ui.Box extends global.laya.ui.Component
	...
	*package
	*{
		*import global.laya.display.Animation;
		*import global.laya.net.Loader;
		*import global.laya.utils.Handler;
	...
	*
	*@example
	*import Animation=global.laya.display.Animation;
	...
	*/
	//class global.laya.display.Animation extends global.laya.display.AnimationPlayerBase
	var Animation=(function(_super){
		...
		__proto.destroy=function(destroyChild){
			(destroyChild===void 0)&& (destroyChild=true);
			this.stop();
			global.laya.display.Sprite.prototype.destroy.call(this,destroyChild);
			this._frames=null;
			this._labels=null;
		}
	...
	*关键帧动画播放类。
	*/
	//class global.laya.display.FrameAnimation extends global.laya.display.AnimationPlayerBase
	...
	*/
	//class global.laya.display.Input extends global.laya.display.Text
...
/**
	...
	*Laya.init(640,800);//设置游戏画布宽高、渲染模式。
	*Laya.stage.bgColor="#efefef";//设置画布的背景颜色。
	*Laya.loader.load("resource/ui/button.png",global.laya.utils.Handler.create(this,loadComplete));//加载资源
	*function loadComplete()
	*{
		*console.log("资源加载完成！");
		*var button=new global.laya.ui.Button("resource/ui/button.png","label");//创建一个 Button 类的实例对象 button ,传入它的皮肤skin和标签label。
		*button.x=100;//设置 button 对象的属性 x 的值，用于控制 button 对象的显示位置。
		*button.y=100;//设置 button 对象的属性 y 的值，用于控制 button 对象的显示位置。
		*button.clickHandler=global.laya.utils.Handler.create(this,onClickButton,[button],false);//设置 button 的点击事件处理函数。
		*Laya.stage.addChild(button);//将此 button 对象添加到显示列表。
		*}
	...
	*{
		...
		*}
	*@example
	*import Button=global.laya.ui.Button;
	*import Handler=global.laya.utils.Handler;
	*class Button_Example{
		*constructor()
		*{
			*Laya.init(640,800);
			*Laya.stage.bgColor="#efefef";//设置画布的背景颜色。
			*Laya.loader.load("resource/ui/button.png",global.laya.utils.Handler.create(this,this.onLoadComplete));//加载资源。
			*}
		...
		*}
	*/
	//class global.laya.ui.Button extends global.laya.ui.Component
	var Button=(function(_super){
		...
		*<p>描边颜色，以字符串表示。</p>
		*默认值为 "#000000"（黑色）;
		*@see global.laya.display.Text.strokeColor()
		*/
		...
		*/
		...
		/**
		*表示按钮文本标签的字体大小。
		*@see global.laya.display.Text.fontSize()
		*/
		...
		*<p>描边宽度（以像素为单位）。</p>
		*默认值0，表示不描边。
		*@see global.laya.display.Text.stroke()
		*/
		...
		/**
		*表示按钮文本标签是否为粗体字。
		*@see global.laya.display.Text.bold()
		*/
		...
		*@see global.laya.display.Text.font()
		*/
		...
		*<ul><li>例如："4,4,4,4,1"</li></ul></p>
		*@see global.laya.ui.AutoBitmap.sizeGrid
		*/
		__getset(0,__proto,'sizeGrid',function(){
	...
	*Laya.stage.bgColor="#efefef";//设置画布的背景颜色
	*var clip;
	*Laya.loader.load("resource/ui/clip_num.png",global.laya.utils.Handler.create(this,loadComplete));//加载资源
	*function loadComplete(){
		*console.log("资源加载完成！");
		*clip=new global.laya.ui.Clip("resource/ui/clip_num.png",10,1);//创建一个 Clip 类的实例对象 clip ,传入它的皮肤skin和横向分割数量、竖向分割数量。
		*clip.autoPlay=true;//设置 clip 动画自动播放。
		*clip.interval=100;//设置 clip 动画的播放时间间隔。
		*clip.x=100;//设置 clip 对象的属性 x 的值，用于控制 clip 对象的显示位置。
		*clip.y=100;//设置 clip 对象的属性 y 的值，用于控制 clip 对象的显示位置。
	...
	*{
		...
		*}
	*@example
	*import Clip=global.laya.ui.Clip;
	*import Handler=global.laya.utils.Handler;
	*class Clip_Example {
		*private clip:Clip;
		*constructor(){
		...
		*private onInit():void {
			...
			*this.clip.on(global.laya.events.Event.CLICK,this,this.onClick);//给 clip 添加点击事件函数侦听。
	...
	*
	*/
	//class global.laya.ui.Clip extends global.laya.ui.Component
	var Clip=(function(_super){
		...
		*/
		...
		/**
		*@copy global.laya.ui.Image#skin
		*/
		__getset(0,__proto,'skin',function(){
			return this._skin;
			},function(value){
	...
	*package
	*{
		*import global.laya.ui.ColorPicker;
		*import global.laya.utils.Handler;
	...
	*Laya.init(640,800);//设置游戏画布宽高
	*Laya.stage.bgColor="#efefef";//设置画布的背景颜色
	*Laya.loader.load("resource/ui/color.png",global.laya.utils.Handler.create(this,loadComplete));//加载资源
	*function loadComplete()
	*{
		*console.log("资源加载完成！");
		*var colorPicket=new global.laya.ui.ColorPicker();//创建一个 ColorPicker 类的实例对象 colorPicket 。
		*colorPicket.skin="resource/ui/color.png";//设置 colorPicket 的皮肤。
		*colorPicket.x=100;//设置 colorPicket 对象的属性 x 的值，用于控制 colorPicket 对象的显示位置。
		*colorPicket.y=100;//设置 colorPicket 对象的属性 y 的值，用于控制 colorPicket 对象的显示位置。
		*colorPicket.changeHandler=global.laya.utils.Handler.create(this,onChangeColor,[colorPicket],false);//设置 colorPicket 的颜色改变回调函数。
		*Laya.stage.addChild(colorPicket);//将此 colorPicket 对象添加到显示列表。
		*}
	...
	*{
		...
		*}
	*@example
	*import ColorPicker=global.laya.ui.ColorPicker;
	*import Handler=global.laya.utils.Handler;
	*class ColorPicker_Example {
		...
		*}
	*/
	//class global.laya.ui.ColorPicker extends global.laya.ui.Component
	var ColorPicker=(function(_super){
		...
		*/
		...
		/**
		*@copy global.laya.ui.Button#skin
		*/
		__getset(0,__proto,'skin',function(){
	...
	*package
	*{
		*import global.laya.ui.ComboBox;
		*import global.laya.utils.Handler;
	...
	*function loadComplete(){
		*console.log("资源加载完成！");
		*var comboBox=new global.laya.ui.ComboBox("resource/ui/button.png","item0,item1,item2,item3,item4,item5");//创建一个 ComboBox 类的实例对象 comboBox ,传入它的皮肤和标签集。
		*comboBox.x=100;//设置 comboBox 对象的属性 x 的值，用于控制 comboBox 对象的显示位置。
		*comboBox.y=100;//设置 comboBox 对象的属性 x 的值，用于控制 comboBox 对象的显示位置。
		*comboBox.selectHandler=new global.laya.utils.Handler(this,onSelect);//设置 comboBox 选择项改变时执行的处理器。
		*Laya.stage.addChild(comboBox);//将此 comboBox 对象添加到显示列表。
		*}
	...
	*{
		...
		*}
	*@example
	*import ComboBox=global.laya.ui.ComboBox;
	*import Handler=global.laya.utils.Handler;
	...
	*
	*/
	//class global.laya.ui.ComboBox extends global.laya.ui.Component
	var ComboBox=(function(_super){
		...
		*/
		...
		/**
		*表示按钮的状态值。
		*@see global.laya.ui.Button#stateNum
		*/
		__getset(0,__proto,'stateNum',function(){
	...
	*
	*@see global.laya.ui.VScrollBar
	*@see global.laya.ui.HScrollBar
	*/
	//class global.laya.ui.ScrollBar extends global.laya.ui.Component
	var ScrollBar=(function(_super){
		...
		*/
		...
		/**
		*设置滚动对象。
		*@see global.laya.ui.TouchScroll#target
		*/
	...
	*
	*@see global.laya.ui.HSlider
	*@see global.laya.ui.VSlider
	*/
	//class global.laya.ui.Slider extends global.laya.ui.Component
	var Slider=(function(_super){
		...
		*/
		__proto.showValueText=function(){
			if (this.showLabel){
				var label=global.laya.ui.Slider.label;
				this.addChild(label);
		...
		*/
		__proto.hideValueText=function(){
			global.laya.ui.Slider.label && global.laya.ui.Slider.label.removeSelf();
		}
	...
	*onInit();
	*function onInit(){
		*var bg=new global.laya.ui.Image("resource/ui/bg.png");//创建一个 Image 类的实例对象 bg ,并传入它的皮肤。
		*bg.x=100;//设置 bg 对象的属性 x 的值，用于控制 bg 对象的显示位置。
		*bg.y=100;//设置 bg 对象的属性 y 的值，用于控制 bg 对象的显示位置。
		*bg.sizeGrid="40,10,5,10";//设置 bg 对象的网格信息。
		*bg.width=150;//设置 bg 对象的宽度。
		*bg.height=250;//设置 bg 对象的高度。
		*Laya.stage.addChild(bg);//将此 bg 对象添加到显示列表。
		*var image=new global.laya.ui.Image("resource/ui/image.png");//创建一个 Image 类的实例对象 image ,并传入它的皮肤。
		*image.x=100;//设置 image 对象的属性 x 的值，用于控制 image 对象的显示位置。
		*image.y=100;//设置 image 对象的属性 y 的值，用于控制 image 对象的显示位置。
		*Laya.stage.addChild(image);//将此 image 对象添加到显示列表。
		*}
	...
	*class Image_Example {
		*constructor(){
			...
			*}
		*private onInit():void {
			*var bg:global.laya.ui.Image=new global.laya.ui.Image("resource/ui/bg.png");//创建一个 Image 类的实例对象 bg ,并传入它的皮肤。
			*bg.x=100;//设置 bg 对象的属性 x 的值，用于控制 bg 对象的显示位置。
			*bg.y=100;//设置 bg 对象的属性 y 的值，用于控制 bg 对象的显示位置。
			*bg.sizeGrid="40,10,5,10";//设置 bg 对象的网格信息。
			*bg.width=150;//设置 bg 对象的宽度。
			*bg.height=250;//设置 bg 对象的高度。
			*Laya.stage.addChild(bg);//将此 bg 对象添加到显示列表。
			*var image:global.laya.ui.Image=new global.laya.ui.Image("resource/ui/image.png");//创建一个 Image 类的实例对象 image ,并传入它的皮肤。
			*image.x=100;//设置 image 对象的属性 x 的值，用于控制 image 对象的显示位置。
			*image.y=100;//设置 image 对象的属性 y 的值，用于控制 image 对象的显示位置。
			*Laya.stage.addChild(image);//将此 image 对象添加到显示列表。
			*}
		*}
	*@see global.laya.ui.AutoBitmap
	*/
	//class global.laya.ui.Image extends global.laya.ui.Component
	var Image=(function(_super){
		...
		*/
		...
		/**
		*@copy global.laya.ui.AutoBitmap#source
		*/
		__getset(0,__proto,'source',function(){
	...
	*onInit();
	*function onInit(){
		*var label=new global.laya.ui.Label();//创建一个 Label 类的实例对象 label 。
		*label.font="Arial";//设置 label 的字体。
		*label.bold=true;//设置 label 显示为粗体。
		*label.leading=4;//设置 label 的行间距。
		*label.wordWrap=true;//设置 label 自动换行。
		...
		*label.height=200;//设置 label 的高度。
		*Laya.stage.addChild(label);//将 label 添加到显示列表。
		*var passwordLabel=new global.laya.ui.Label("请原谅我，我不想被人看到我心里话。");//创建一个 Label 类的实例对象 passwordLabel 。
		*passwordLabel.asPassword=true;//设置 passwordLabel 的显示反式为密码显示。
		*passwordLabel.x=100;//设置 passwordLabel 对象的属性 x 的值，用于控制 passwordLabel 对象的显示位置。
		*passwordLabel.y=350;//设置 passwordLabel 对象的属性 y 的值，用于控制 passwordLabel 对象的显示位置。
		*passwordLabel.width=300;//设置 passwordLabel 的宽度。
		...
		*}
	*@example
	*import Label=global.laya.ui.Label;
	*class Label_Example {
		*constructor(){
			*Laya.init(640,800);//设置游戏画布宽高。
			*Laya.stage.bgColor="#efefef";//设置画布的背景颜色。
		...
		*private onInit():void {
			...
			*}
		*}
	*@see global.laya.display.Text
	*/
	//class global.laya.ui.Label extends global.laya.ui.Component
	var Label=(function(_super){
		...
		/**@copy global.laya.display.Text#changeText()
		**/
		__proto.changeText=function(text){
		...
		*@see global.laya.display.Text.padding
		*/
		__getset(0,__proto,'padding',function(){
		...
		/**
		*@copy global.laya.display.Text#bold
		*/
		__getset(0,__proto,'bold',function(){
		...
		/**
		*@copy global.laya.display.Text#align
		*/
		__getset(0,__proto,'align',function(){
		...
		/**
		*当前文本内容字符串。
		*@see global.laya.display.Text.text
		*/
		__getset(0,__proto,'text',function(){
		...
		/**
		*@copy global.laya.display.Text#italic
		*/
		__getset(0,__proto,'italic',function(){
		...
		/**
		*@copy global.laya.display.Text#wordWrap
		*/
		/**
		*@copy global.laya.display.Text#wordWrap
		*/
		...
		/**
		*@copy global.laya.display.Text#font
		*/
		__getset(0,__proto,'font',function(){
		...
		/**
		*@copy global.laya.display.Text#color
		*/
		__getset(0,__proto,'color',function(){
		...
		/**
		*@copy global.laya.display.Text#valign
		*/
		__getset(0,__proto,'valign',function(){
		...
		/**
		*@copy global.laya.display.Text#leading
		*/
		__getset(0,__proto,'leading',function(){
		...
		/**
		*@copy global.laya.display.Text#fontSize
		*/
		__getset(0,__proto,'fontSize',function(){
		...
		/**
		*@copy global.laya.display.Text#bgColor
		*/
		__getset(0,__proto,'bgColor',function(){
		...
		/**
		*@copy global.laya.display.Text#borderColor
		*/
		__getset(0,__proto,'borderColor',function(){
		...
		/**
		*@copy global.laya.display.Text#stroke
		*/
		__getset(0,__proto,'stroke',function(){
		...
		/**
		*@copy global.laya.display.Text#strokeColor
		*/
		__getset(0,__proto,'strokeColor',function(){
		...
		*/
		...
		/**
		*@copy global.laya.display.Text#overflow
		*/
		/**
		*@copy global.laya.display.Text#overflow
		*/
		...
		/**
		*@copy global.laya.display.Text#underline
		*/
		/**
		*@copy global.laya.display.Text#underline
		*/
		...
		/**
		*@copy global.laya.display.Text#underlineColor
		*/
		/**
		*@copy global.laya.display.Text#underlineColor
		*/
	...
	*package
	*{
		*import global.laya.ui.ProgressBar;
		*import global.laya.utils.Handler;
	...
	*Laya.loader.load(res,global.laya.utils.Handler.create(this,onLoadComplete));//加载资源。
	*function onLoadComplete()
	*{
		*progressBar=new global.laya.ui.ProgressBar("resource/ui/progress.png");//创建一个 ProgressBar 类的实例对象 progressBar 。
		*progressBar.x=100;//设置 progressBar 对象的属性 x 的值，用于控制 progressBar 对象的显示位置。
		*progressBar.y=100;//设置 progressBar 对象的属性 y 的值，用于控制 progressBar 对象的显示位置。
		*progressBar.value=0.3;//设置 progressBar 的进度值。
		*progressBar.width=200;//设置 progressBar 的宽度。
		...
		*progressBar.changeHandler=new global.laya.utils.Handler(this,onChange);//设置 progressBar 的value值改变时执行的处理器。
		*Laya.stage.addChild(progressBar);//将 progressBar 添加到显示列表。
		*Laya.timer.once(3000,this,changeValue);//设定 3000ms（毫秒）后，执行函数changeValue。
		*}
	...
	*{
		...
		*}
	*@example
	*import ProgressBar=global.laya.ui.ProgressBar;
	*import Handler=global.laya.utils.Handler;
	*class ProgressBar_Example {
		...
		*}
	*/
	//class global.laya.ui.ProgressBar extends global.laya.ui.Component
	...
	var Input=(function(_super){
		...
		*/
		__proto.setSelection=function(startIndex,endIndex){
			this.focus=true;
			global.laya.display.Input.inputElement.setSelection(startIndex,endIndex);
		}
		...
		/**@inheritDoc*/
		__proto.render=function(context,x,y){
			global.laya.display.Sprite.prototype.render.call(this,context,x,y);
		}
		...
		*/
		...
		__proto._setInputMethod=function(){
			Input.input.setVisible(false);
			Input.area.setVisible(false);
			Input.inputElement=(this._multiline ? Input.area :Input.input);
		}

		__proto._focusIn=function(){
			global.laya.display.Input.isInputting=true;
			var input=this.nativeInput;
			input.setRestrict(this._restrictPattern);
			this._focus=true;
			var cssStyle=input;
		...
		__proto._focusOut=function(){
			global.laya.display.Input.isInputting=false;
			this._focus=false;
			this._text=null;
		...
		*/
		...
		Input._popupInputMethod=function(e){
			if (!global.laya.display.Input.isInputting)return;
			var input=global.laya.display.Input.inputElement;
			input.focus();
		}

		Input._createInputElement=function(){
			Input.area=InputWrap.createByData(Mini.pageData["textAreaContainerStyle"]);
			Input.input=InputWrap.createByData(Mini.pageData["inputContainerStyle"]);
		}

		Input._processInputting=function(e){
			var input=global.laya.display.Input.inputElement.target;
			if (!input)return;
			var value=global.laya.display.Input.inputElement.value;
			if (input._restrictPattern){
				value=value.replace(/\u2006|\x27/g,"");
				if (input._restrictPattern.test(value)){
					value=value.replace(input._restrictPattern,"");
					global.laya.display.Input.inputElement.value=value;
				}
			}
			input._text=value;
			input.event("input");
	...
	*/
	//class global.laya.resource.HTMLImage extends global.laya.resource.FileBitmap
	...
	*/
	//class global.laya.ui.View extends global.laya.ui.Box
	var View=(function(_super){
		...
		*/
		__proto.destroy=function(destroyChild){
			...
			this._idMap=null;
			this._aniList=null;
			global.laya.ui.Component.prototype.destroy.call(this,destroyChild);
		}
				...
				for (var i=0,n=child.length;i < n;i++){
					...
					if (comp.hasOwnProperty("itemRender")&& (node.props.name=="render" || node.props.renderType==="render")){
						...
						if (node.type=="Script"){
							...
							tChild instanceof global.laya.display.Sprite )&& comp.addChild(tChild);
						}
					}
				}
			};
	...
	*/
	//class global.laya.ui.LayoutBox extends global.laya.ui.Box
	var LayoutBox=(function(_super){
		...
		__proto.addChild=function(child){
			...
			return global.laya.display.Node.prototype.addChild.call(this,child);
		}
		...
		__proto.addChildAt=function(child,index){
			child.on("resize",this,this.onResize);
			this._setItemChanged();
			return global.laya.display.Node.prototype.addChildAt.call(this,child,index);
		}
		...
		__proto.removeChild=function(child){
			...
			this._setItemChanged();
			return global.laya.display.Node.prototype.removeChild.call(this,child);
		}
		...
		__proto.removeChildAt=function(index){
			...
			this._setItemChanged();
			return global.laya.display.Node.prototype.removeChildAt.call(this,index);
		}
	...
	*Laya.loader.load(res,new global.laya.utils.Handler(this,onLoadComplete));//加载资源。
	*function onLoadComplete(){
		*var arr=[];//创建一个数组，用于存贮列表的数据信息。
		*for (var i=0;i &lt;20;i++){
			*arr.push({label:"item"+i});
			*}
		*var list=new global.laya.ui.List();//创建一个 List 类的实例对象 list 。
		...
		*list.pos(100,100);//设置 list 的位置。
		*list.selectEnable=true;//设置 list 可选。
		*list.selectHandler=new global.laya.utils.Handler(this,onSelect);//设置 list 改变选择项执行的处理器。
		*Laya.stage.addChild(list);//将 list 添加到显示列表。
		*}
	*function onSelect(index)
	*{
	...
	*
	*@example
	*import List=global.laya.ui.List;
	*import Handler=global.laya.utils.Handler;
	*public class List_Example {
		*public List_Example(){
			*Laya.init(640,800);//设置游戏画布宽高。
	...
	*class Item extends Box {
		...
		*}
	*/
	//class global.laya.ui.List extends global.laya.ui.Box
	var List=(function(_super){
		...
		*/
		...
		/**@inheritDoc */
		__proto.changeSize=function(){
			global.laya.ui.Component.prototype.changeSize.call(this);
	...
	*/
	//class global.laya.ui.Panel extends global.laya.ui.Box
	var Panel=(function(_super){
		...
		/**@inheritDoc */
		__proto.createChildren=function(){
			global.laya.display.Node.prototype.addChild.call(this,this._content=new Box());
		}
		...
		*/
		__getset(0,__proto,'hScrollBarSkin',function(){
			...
			if (this._hScrollBar==null){
				global.laya.display.Node.prototype.addChild.call(this,this._hScrollBar=new HScrollBar());
		...
		*/
		__getset(0,__proto,'vScrollBarSkin',function(){
			...
			if (this._vScrollBar==null){
				global.laya.display.Node.prototype.addChild.call(this,this._vScrollBar=new VScrollBar());
	...
	*/
	//class global.laya.ui.UIGroup extends global.laya.ui.Box
	var UIGroup=(function(_super){
		...
		*/
		...
		/**
		*@copy global.laya.ui.Button#labelColors()
		*/
		__getset(0,__proto,'labelColors',function(){
	...
	*package
	*{
		*import global.laya.ui.Tree;
		*import global.laya.utils.Browser;
		*import global.laya.utils.Handler;
	...
	*Laya.loader.load(res,new global.laya.utils.Handler(this,onLoadComplete));
	*function onLoadComplete(){
		*var xmlString;//创建一个xml字符串，用于存储树结构数据。
		*xmlString="&lt;root&gt;&lt;item label='box1'&gt;&lt;abc label='child1'/&gt;&lt;abc label='child2'/&gt;&lt;abc label='child3'/&gt;&lt;abc label='child4'/&gt;&lt;abc label='child5'/&gt;&lt;/item&gt;&lt;item label='box2'&gt;&lt;abc label='child1'/&gt;&lt;abc label='child2'/&gt;&lt;abc label='child3'/&gt;&lt;abc label='child4'/&gt;&lt;/item&gt;&lt;/root&gt;";
		*var domParser=new global.laya.utils.Browser.window.DOMParser();//创建一个DOMParser实例domParser。
		*var xml=domParser.parseFromString(xmlString,"text/xml");//解析xml字符。
		*var tree=new global.laya.ui.Tree();//创建一个 Tree 类的实例对象 tree 。
		*tree.scrollBarSkin="resource/ui/vscroll.png";//设置 tree 的皮肤。
	...
	*(function (_super){
		*function Item(){
			...
			*this.right=0;
			*this.left=0;
			*var selectBox=new global.laya.ui.Clip("resource/ui/clip_selectBox.png",1,2);
			...
			*var folder=new global.laya.ui.Clip("resource/ui/clip_tree_folder.png",1,3);
			*folder.name="folder";//设置 folder 的name 为“folder”时，将被识别为树结构的文件夹开启状态图表。2帧：折叠状态、打开状态。
			*folder.x=14;
			*folder.y=4;
			*this.addChild(folder);
			*var label=new global.laya.ui.Label("treeItem");
			...
			*label.right=0;
			*this.addChild(label);
			*var arrow=new global.laya.ui.Clip("resource/ui/clip_tree_arrow.png",1,2);
		...
		*})(global.laya.ui.Box);
	*@example
	*import Tree=global.laya.ui.Tree;
	*import Browser=global.laya.utils.Browser;
	*import Handler=global.laya.utils.Handler;
	*class Tree_Example {
		*constructor(){
			*Laya.init(640,800);
	...
	*class Item extends Box {
		...
		*}
	*/
	//class global.laya.ui.Tree extends global.laya.ui.Box
	...
	*/
	//class global.laya.ui.ViewStack extends global.laya.ui.Box
	...
	*@private
	*/
	//class global.laya.utils.GraphicAnimation extends global.laya.display.FrameAnimation
	...
	*package
	*{
		*import global.laya.ui.CheckBox;
		*import global.laya.utils.Handler;
	...
	*Laya.init(640,800);//设置游戏画布宽高
	*Laya.stage.bgColor="#efefef";//设置画布的背景颜色
	*Laya.loader.load("resource/ui/check.png",global.laya.utils.Handler.create(this,loadComplete));//加载资源
	*function loadComplete()
	*{
		*console.log("资源加载完成！");
		*var checkBox:global.laya.ui.CheckBox=new global.laya.ui.CheckBox("resource/ui/check.png","这个是一个CheckBox组件。");//创建一个 CheckBox 类的类的实例对象 checkBox ,传入它的皮肤skin和标签label。
		*checkBox.x=100;//设置 checkBox 对象的属性 x 的值，用于控制 checkBox 对象的显示位置。
		*checkBox.y=100;//设置 checkBox 对象的属性 y 的值，用于控制 checkBox 对象的显示位置。
		*checkBox.clickHandler=new global.laya.utils.Handler(this,this.onClick,[checkBox],false);//设置 checkBox 的点击事件处理器。
		*Laya.stage.addChild(checkBox);//将此 checkBox 对象添加到显示列表。
		*}
	...
	*{
		...
		*}
	*@example
	*import CheckBox=global.laya.ui.CheckBox;
	*import Handler=global.laya.utils.Handler;
	*class CheckBox_Example{
		...
		*}
	*/
	//class global.laya.ui.CheckBox extends global.laya.ui.Button
	var CheckBox=(function(_super){
		...
		*/
		...
		/**@inheritDoc */
		__proto.preinitialize=function(){
			global.laya.ui.Component.prototype.preinitialize.call(this);
			this.toggle=true;
	...
	*/
	//class global.laya.ui.FontClip extends global.laya.ui.Clip
	...
	*package
	*{
		*import global.laya.ui.HScrollBar;
		*import global.laya.utils.Handler;
	...
	*function onLoadComplete(){
		*console.log("资源加载完成！");
		*hScrollBar=new global.laya.ui.HScrollBar();//创建一个 HScrollBar 类的实例对象 hScrollBar 。
		*hScrollBar.skin="resource/ui/hscroll.png";//设置 hScrollBar 的皮肤。
		*hScrollBar.x=100;//设置 hScrollBar 对象的属性 x 的值，用于控制 hScrollBar 对象的显示位置。
		*hScrollBar.y=100;//设置 hScrollBar 对象的属性 y 的值，用于控制 hScrollBar 对象的显示位置。
		*hScrollBar.changeHandler=new global.laya.utils.Handler(this,onChange);//设置 hScrollBar 的滚动变化处理器。
		*Laya.stage.addChild(hScrollBar);//将此 hScrollBar 对象添加到显示列表。
		*}
	*function onChange(value)
	*{
		...
		*}
	*@example
	*import HScrollBar=global.laya.ui.HScrollBar;
	*import Handler=global.laya.utils.Handler;
	*class HScrollBar_Example {
		...
		*}
	*/
	//class global.laya.ui.HScrollBar extends global.laya.ui.ScrollBar
	...
	*Laya.loader.load(res,global.laya.utils.Handler.create(this,onLoadComplete));
	*function onLoadComplete(){
		*console.log("资源加载完成！");
		*hSlider=new global.laya.ui.HSlider();//创建一个 HSlider 类的实例对象 hSlider 。
		*hSlider.skin="resource/ui/hslider.png";//设置 hSlider 的皮肤。
		*hSlider.min=0;//设置 hSlider 最低位置值。
		*hSlider.max=10;//设置 hSlider 最高位置值。
		*hSlider.value=2;//设置 hSlider 当前位置值。
		...
		*hSlider.x=100;//设置 hSlider 对象的属性 x 的值，用于控制 hSlider 对象的显示位置。
		*hSlider.y=100;//设置 hSlider 对象的属性 y 的值，用于控制 hSlider 对象的显示位置。
		*hSlider.changeHandler=new global.laya.utils.Handler(this,onChange);//设置 hSlider 位置变化处理器。
		*Laya.stage.addChild(hSlider);//把 hSlider 添加到显示列表。
		*}
	*function onChange(value)
	*{
	...
	*@example
	*import Handler=global.laya.utils.Handler;
	*import HSlider=global.laya.ui.HSlider;
	*class HSlider_Example {
		...
		*}
	*
	*@see global.laya.ui.Slider
	*/
	//class global.laya.ui.HSlider extends global.laya.ui.Slider
	...
	*@see global.laya.ui.RadioGroup
	*/
	//class global.laya.ui.Radio extends global.laya.ui.Button
	...
	*package
	*{
		*import global.laya.ui.vScrollBar;
		*import global.laya.ui.VScrollBar;
		*import global.laya.utils.Handler;
	...
	*Laya.loader.load(res,global.laya.utils.Handler.create(this,onLoadComplete));//加载资源。
	*function onLoadComplete(){
		*vScrollBar=new global.laya.ui.VScrollBar();//创建一个 vScrollBar 类的实例对象 hScrollBar 。
		*vScrollBar.skin="resource/ui/vscroll.png";//设置 vScrollBar 的皮肤。
		*vScrollBar.x=100;//设置 vScrollBar 对象的属性 x 的值，用于控制 vScrollBar 对象的显示位置。
		*vScrollBar.y=100;//设置 vScrollBar 对象的属性 y 的值，用于控制 vScrollBar 对象的显示位置。
		*vScrollBar.changeHandler=new global.laya.utils.Handler(this,onChange);//设置 vScrollBar 的滚动变化处理器。
		*Laya.stage.addChild(vScrollBar);//将此 vScrollBar 对象添加到显示列表。
		*}
	*function onChange(value){
		*console.log("滚动条的位置： value="+value);
		*}
	*@example
	*import VScrollBar=global.laya.ui.VScrollBar;
	*import Handler=global.laya.utils.Handler;
	*class VScrollBar_Example {
		...
		*}
	*/
	//class global.laya.ui.VScrollBar extends global.laya.ui.ScrollBar
	...
	*package
	*{
		*import global.laya.display.Stage;
		*import global.laya.ui.TextInput;
		*import global.laya.utils.Handler;
	...
	*Laya.init(640,800);//设置游戏画布宽高
	*Laya.stage.bgColor="#efefef";//设置画布的背景颜色
	*Laya.loader.load(["resource/ui/input.png"],global.laya.utils.Handler.create(this,onLoadComplete));//加载资源。
	*function onLoadComplete(){
		*var textInput=new global.laya.ui.TextInput("这是一个TextInput实例。");//创建一个 TextInput 类的实例对象 textInput 。
		*textInput.skin="resource/ui/input.png";//设置 textInput 的皮肤。
		*textInput.sizeGrid="4,4,4,4";//设置 textInput 的网格信息。
		*textInput.color="#008fff";//设置 textInput 的文本颜色。
		*textInput.font="Arial";//设置 textInput 的文本字体。
		...
		*}
	*@example
	*import Stage=global.laya.display.Stage;
	*import TextInput=global.laya.ui.TextInput;
	*import Handler=global.laya.utils.Handler;
	*class TextInput_Example {
		...
		*}
	*/
	//class global.laya.ui.TextInput extends global.laya.ui.Label
	var TextInput=(function(_super){
		...
		*/
		...
		/**
		*@copy global.laya.display.Input#prompt
		*/
		__getset(0,__proto,'prompt',function(){
		...
		/**
		*@copy global.laya.display.Input#promptColor
		*/
		__getset(0,__proto,'promptColor',function(){
		...
		/**
		*@copy global.laya.display.Input#maxChars
		*/
		__getset(0,__proto,'maxChars',function(){
		...
		/**
		*@copy global.laya.display.Input#focus
		*/
		__getset(0,__proto,'focus',function(){
		...
		/**
		*@copy global.laya.display.Input#type
		*/
		__getset(0,__proto,'type',function(){
		...
		/**
		*@copy global.laya.display.Input#asPassword
		*/
		__getset(0,__proto,'asPassword',function(){
	...
	*Laya.stage.bgColor="#efefef";//设置画布的背景颜色
	*var vSlider;
	*Laya.loader.load(["resource/ui/vslider.png","resource/ui/vslider$bar.png"],global.laya.utils.Handler.create(this,onLoadComplete));//加载资源。
	*function onLoadComplete(){
		*vSlider=new global.laya.ui.VSlider();//创建一个 VSlider 类的实例对象 vSlider 。
		*vSlider.skin="resource/ui/vslider.png";//设置 vSlider 的皮肤。
		*vSlider.min=0;//设置 vSlider 最低位置值。
		*vSlider.max=10;//设置 vSlider 最高位置值。
		*vSlider.value=2;//设置 vSlider 当前位置值。
		...
		*vSlider.x=100;//设置 vSlider 对象的属性 x 的值，用于控制 vSlider 对象的显示位置。
		*vSlider.y=100;//设置 vSlider 对象的属性 y 的值，用于控制 vSlider 对象的显示位置。
		*vSlider.changeHandler=new global.laya.utils.Handler(this,onChange);//设置 vSlider 位置变化处理器。
		*Laya.stage.addChild(vSlider);//把 vSlider 添加到显示列表。
		*}
	*function onChange(value){
		*console.log("滑块的位置： value="+value);
	...
	*@example
	*import HSlider=global.laya.ui.HSlider;
	*import VSlider=global.laya.ui.VSlider;
	*import Handler=global.laya.utils.Handler;
	...
	*@see global.laya.ui.Slider
	*/
	//class global.laya.ui.VSlider extends global.laya.ui.Slider
	...
	//class ui.TestPageSimpleUI extends global.laya.ui.View
	var TestPageSimpleUI=(function(_super){
		function TestPageSimpleUI(){
			this.btn=null;
			this.clip=null;
			this.combobox=null;
			this.tab=null;
	...
	*/
	//class global.laya.ui.HBox extends global.laya.ui.LayoutBox
	...
	*/
	//class global.laya.ui.VBox extends global.laya.ui.LayoutBox
	...
	*package
	*{
		*import global.laya.ui.Radio;
		*import global.laya.ui.RadioGroup;
		*import global.laya.utils.Handler;
	...
	*Laya.init(640,800);//设置游戏画布宽高、渲染模式
	*Laya.stage.bgColor="#efefef";//设置画布的背景颜色
	*Laya.loader.load(["resource/ui/radio.png"],global.laya.utils.Handler.create(this,onLoadComplete));
	*function onLoadComplete(){
		*var radioGroup=new global.laya.ui.RadioGroup();//创建一个 RadioGroup 类的实例对象 radioGroup 。
		*radioGroup.pos(100,100);//设置 radioGroup 的位置信息。
		*radioGroup.labels="item0,item1,item2";//设置 radioGroup 的标签集。
		*radioGroup.skin="resource/ui/radio.png";//设置 radioGroup 的皮肤。
		*radioGroup.space=10;//设置 radioGroup 的项间隔距离。
		*radioGroup.selectHandler=new global.laya.utils.Handler(this,onSelect);//设置 radioGroup 的选择项发生改变时执行的处理器。
		*Laya.stage.addChild(radioGroup);//将 radioGroup 添加到显示列表。
		*}
	*function onSelect(index){
		...
		*}
	*@example
	*import Radio=global.laya.ui.Radio;
	*import RadioGroup=global.laya.ui.RadioGroup;
	*import Handler=global.laya.utils.Handler;
	*class RadioGroup_Example {
		...
		*}
	*/
	//class global.laya.ui.RadioGroup extends global.laya.ui.UIGroup
	...
	*Laya.init(640,800);//设置游戏画布宽高
	*Laya.stage.bgColor="#efefef";//设置画布的背景颜色
	*Laya.loader.load(["resource/ui/tab.png"],global.laya.utils.Handler.create(this,onLoadComplete));
	*function onLoadComplete(){
		*var tab=new global.laya.ui.Tab();//创建一个 Tab 类的实例对象 tab 。
		*tab.skin="resource/ui/tab.png";//设置 tab 的皮肤。
		*tab.labels="item0,item1,item2";//设置 tab 的标签集。
		*tab.x=100;//设置 tab 对象的属性 x 的值，用于控制 tab 对象的显示位置。
		*tab.y=100;//设置 tab 对象的属性 y 的值，用于控制 tab 对象的显示位置。
		*tab.selectHandler=new global.laya.utils.Handler(this,onSelect);//设置 tab 的选择项发生改变时执行的处理器。
		*Laya.stage.addChild(tab);//将 tab 添到显示列表。
		*}
	*function onSelect(index){
	...
	*class Tab_Example {
		...
		*}
	*/
	//class global.laya.ui.Tab extends global.laya.ui.UIGroup
	...
	*package
	*{
		*import global.laya.ui.TextArea;
		*import global.laya.utils.Handler;
	...
	*Laya.loader.load(["resource/ui/input.png"],global.laya.utils.Handler.create(this,onLoadComplete));//加载资源。
	*function onLoadComplete(){
		*var textArea=new global.laya.ui.TextArea("这个一个TextArea实例。");//创建一个 TextArea 类的实例对象 textArea 。
		*textArea.skin="resource/ui/input.png";//设置 textArea 的皮肤。
		*textArea.sizeGrid="4,4,4,4";//设置 textArea 的网格信息。
		*textArea.color="#008fff";//设置 textArea 的文本颜色。
		*textArea.font="Arial";//设置 textArea 的字体。
		...
		*}
	*@example
	*import TextArea=global.laya.ui.TextArea;
	*import Handler=global.laya.utils.Handler;
	*class TextArea_Example {
		...
		*}
	*/
	//class global.laya.ui.TextArea extends global.laya.ui.TextInput
	...
	Laya.__init([EventDispatcher,Browser,GraphicAnimation,Render,LoaderManager,LocalStorage,Timer,View]);
	new global.test.TestSimple();
